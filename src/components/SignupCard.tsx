import {
  ...
  const SignupCard = () => {
    ...
    // Validation function to ensure that user input is valid.
    const validateForm = () => {
      let valid = true;
      ...
      if (!email) {
        ...
        valid = false;
      } else if (!email.includes("@")) { // Framework doesn't validate on its own.
          ...
          <Stack spacing={10} pl="5">
            <FormControl size={"lg"} isInvalid={!!errors.email}>
              ...
              <Input
                ...
                value={email}
                onChange={(e) => setEmail(e.target.value)} // Monitors the state of the input field.
              />
            ...
            <FormControl size={"lg"} isInvalid={!!errors.password}>
              ...
              {errors.password && <FormHelperText color="red.500">{errors.password}</FormHelperText>}
              <FormHelperText>Must be between 6 and 30 characters long</FormHelperText>
            </FormControl>
            <FormControl size={"lg"} isInvalid={!!errors.repeatPassword}>
              ...
              />
              {errors.repeatPassword && <FormHelperText color="red.500">{errors.repeatPassword}</FormHelperText>}
            </FormControl>
            <Button
              variant="solid"
              colorScheme="orange"
              width="50%"
              onClick={() => {
                if (validateForm()) { // Only goes through if the form is valid.
